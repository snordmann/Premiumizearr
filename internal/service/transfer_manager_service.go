...
func (t *TransferManagerService) CleanUpUnzipDir() {
	log.Info("TransferManagerService: Cleaning unzip directory")

	unzipBase, err := t.config.GetUnzipBaseLocation()
	if err != nil {
		log.Errorf("TransferManagerService: Error getting unzip base location: %s", err.Error())
		return
	}

	err = utils.RemoveContents(unzipBase)
	if err != nil {
		log.Errorf("TransferManagerService: Error cleaning unzip directory: %s", err.Error())
		return
	}
...
func (manager *TransferManagerService) TaskUpdateTransfersList() {
	log.Debug("TransferManagerService: Running Task UpdateTransfersList")
	transfers, err := manager.premiumizemeClient.GetTransfers()
	if err != nil {
		log.Errorf("TransferManagerService: Error getting transfers: %s", err.Error())
		return
	}
	var foundTransfers []premiumizeme.Transfer

	log.Tracef("TransferManagerService: Checking %d transfers against %d Arr clients", len(transfers), len(manager.arrsManager.GetArrs()))
	for _, transfer := range transfers {
		...
		for _, arr := range manager.arrsManager.GetArrs() {
			...
			log.Tracef("TransferManagerService: Checking transfer %s against %s history", transfer.Name, arr.GetArrName())
			arrID, contains := arr.HistoryContains(transfer.Name)
			if !contains {
				log.Tracef("%s history doesn't contain %s", arr.GetArrName(), transfer.Name)
				continue
			}
			log.Tracef("TransferManagerService: Found %s in %s history", transfer.Name, arr.GetArrName())
			found = true
			foundTransfers = append(foundTransfers, transfer)
			if transfer.Status == "error" {
				log.Debugf("TransferManagerService: Processing transfer that has errored: %s", transfer.Name)
...
func (manager *TransferManagerService) TaskCheckPremiumizeDownloadsFolder() {
	log.Debug("TransferManagerService: Running Task CheckPremiumizeDownloadsFolder")

	items, err := manager.premiumizemeClient.ListFolder(manager.downloadsFolderID)
	if err != nil {
		log.Errorf("TransferManagerService: Error listing downloads folder: %s", err.Error())
		return
	}

	for _, item := range items {
		if manager.countDownloads() < manager.config.SimultaneousDownloads {
			log.Debugf("TransferManagerService: Processing completed item: %s", item.Name)
			manager.HandleFinishedItem(item, manager.config.DownloadsDirectory)
		} else {
			log.Debugf("TransferManagerService: Not processing any more transfers, %d are running and cap is %d", manager.countDownloads(), manager.config.SimultaneousDownloads)
			break
		}
	}
}
...
func (manager *TransferManagerService) HandleFinishedItem(item premiumizeme.Item, downloadDirectory string) {
	if manager.downloadExists(item.Name) {
		log.Tracef("TransferManagerService: Transfer %s is already downloading", item.Name)
		return
	}
	...
	go func() {
		log.Debug("TransferManagerService: Downloading: ", item.Name)
		log.Tracef("TransferManagerService: %+v", item)
		var link string
		var err error
		if item.Type == "file" {
			...
			log.Errorf("TransferManagerService: Item is not of type 'file' or 'folder' !! Can't download %s", item.Name)
			return
		}
		if err != nil {
			log.Errorf("TransferManagerService: Error generating download link: %s", err)
			manager.removeDownload(item.Name)
			return
		}
		log.Trace("TransferManagerService: Downloading from: ", link)

		tempDir, err := manager.config.GetNewUnzipLocation()
		if err != nil {
			log.Errorf("TransferManagerService: Could not create temp dir: %s", err)
			manager.removeDownload(item.Name)
			return
		}
		...
		log.Trace("TransferManagerService: Downloading to: ", savePath)

		out, err := os.Create(savePath)
		if err != nil {
			log.Errorf("TransferManagerService: Could not create save path: %s", err)
			manager.removeDownload(item.Name)
			return
		}
		defer out.Close()
		...
		if err != nil {
			log.Errorf("TransferManagerService: Could not download file: %s", err)
			manager.removeDownload(item.Name)
			return
		}

		log.Tracef("TransferManagerService: Unzipping %s to %s", savePath, downloadDirectory)
		err = utils.Unzip(savePath, downloadDirectory)
		if err != nil {
			log.Errorf("TransferManagerService: Could not unzip file: %s", err)
			manager.removeDownload(item.Name)
			return
		}

		log.Tracef("TransferManagerService: Removing zip %s from system", savePath)
		err = os.RemoveAll(savePath)
		if err != nil {
			manager.removeDownload(item.Name)
			log.Errorf("TransferManagerService: Could not remove zip: %s", err)
			return
		}
		...
		if err != nil {
			manager.removeDownload(item.Name)
			log.Errorf("TransferManagerService: Error deleting folder on premiumize.me: %s", err)
			return
		}
